...
def test_page_delete(client, page_dao_manager):
    ...
    page_dao_manager.cur.execute("INSERT INTO page_index (word, doc_id, frequency) VALUES (?, ?, ?)", ("word", "testid", 5))
    page_dao_manager.delete(page)
    result = page_dao_manager.cur.execute("SELECT * FROM page_index WHERE doc_id=?", (page.id,)).fetchone()
...
def test_update_page_index_id(client, page_dao_manager):
    ...
    page_dao_manager.cur.executemany("INSERT INTO page_index (word, doc_id, frequency) VALUES (?, ?, ?)", sample_rows)
    ...
    updated_results = page_dao_manager.cur.execute("SELECT * FROM page_index WHERE doc_id=?", (new_id,)).fetchall()
    ...
    with app.app_context():
        result_old = page_dao_manager.cur.execute("SELECT * FROM page_index WHERE doc_id=?", (old_id,)).fetchall()
...
def test_delete_old_tokens(client, page_dao_manager):
    ...
    for word, frequency in old_page_index.items():
        page_dao_manager.cur.execute("INSERT INTO page_index (word, doc_id, frequency) VALUES (?, ?, ?)", 
    ...
    for word, frequency in new_page_index.items():
        page_dao_manager.cur.execute("INSERT OR REPLACE INTO page_index (word, doc_id, frequency) VALUES (?, ?, ?)", (word, page.id, frequency))

    # Query the database for the remaining tokens and convert to a dictionary
    remaining_tokens_dict = {
        word: frequency for word, frequency in
        page_dao_manager.cur.execute("SELECT word, frequency FROM page_index WHERE doc_id=?", (page.id,)).fetchall()
    }
...
def test_add_or_update_tokens(client, page_dao_manager):
    ...
    for word, frequency in initial_page_index.items():
        page_dao_manager.cur.execute("INSERT INTO page_index (word, doc_id, frequency) VALUES (?, ?, ?)", (word, page.id, frequency))
